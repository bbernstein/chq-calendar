...
// src/services/eventsCalendarDataSyncService.ts
var eventsCalendarDataSyncService_exports = {};
__export(eventsCalendarDataSyncService_exports, {
  EventsCalendarDataSyncService: () => EventsCalendarDataSyncService,
  default: () => eventsCalendarDataSyncService_default
});
module.exports = __toCommonJS(eventsCalendarDataSyncService_exports);
...
// src/services/eventsCalendarApiClient.ts
var EventsCalendarApiClient = class {
  // 5 minutes
  constructor(baseUrl = "https://www.chq.org/wp-json/tribe/events/v1") {
   ...
   */
  async getAllEventsInRange(dateRange) {
    const allEvents = [];
    let page = 1;
    let hasMore = true;
    const perPage = 100;
    ...
    while (hasMore) {
      try {
        ...
        hasMore = response.next_rest_url !== void 0 && response.events.length === perPage;
        page++;
        if (hasMore) {
   ...
   */
  async getSeasonEvents(year = 2025) {
    ...
    for (const range of monthRanges) {
      try {
        const monthEvents = await this.getAllEventsInRange(range);
        allEvents.push(...monthEvents);
      } catch (error) {
        console.error(`Error fetching events for ${range.start} to ${range.end}:`, error);
      }
    }
    return allEvents.filter((event) => {
      const eventDate = new Date(event.start_date);
      return eventDate >= seasonDates.start && eventDate <= seasonDates.end;
    });
  }
  /**
   ...
   */
  async retryWithBackoff(config, maxRetries) {
...
// src/services/eventsCalendarDataSyncService.ts
var EventsCalendarDataSyncService = class {
   ...
   */
  async syncAllSeasonEvents(year = 2025) {
   ...
   */
  async getExistingEvent(eventId) {
    try {
      return null;
    } catch (error) {
  ...
  async createEvent(event) {
    try {
      console.log(`Creating new event: ${event.title} (ID: ${event.id})`);
    } catch (error) {
   ...
   */
  async updateEvent(existingEvent, newEvent) {
    try {
      const hasChanges = this.detectChanges(existingEvent, newEvent);
      if (hasChanges) {
        console.log(`Updating event: ${newEvent.title} (ID: ${newEvent.id})`);
        return true;
      }
      return false;
    } catch (error) {
   ...
   */
  async cleanupOldEvents(dateRange, currentEvents) {
    try {
...
var eventsCalendarDataSyncService_default = EventsCalendarDataSyncService;
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  EventsCalendarDataSyncService
});
