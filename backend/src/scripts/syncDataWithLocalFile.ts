...
async function main() {
  ...
  try {
    ...
    // Now fetch the cached all-events.json file
    if (saveLocal || process.argv.includes('--fetch-cache')) {
      console.log('\nFetching all-events.json from cache...');
      ...
      const allEvents = await cacheService.get({ filters: {} });
      ...
      if (allEvents && allEvents.data) {
        ...
        if (saveLocal) {
          ...
          console.log(`\nSaved all-events.json to: ${outputPath}`);
        }
      } else {
    ...
    if (process.argv.includes('--fetch-s3-direct')) {
      ...
      const s3Key = `${process.env.CACHE_S3_KEY_PREFIX || 'calendar-cache'}/all-events.json`;
