...
3. **This merge itself deploys the frontend** — verified by running the shipped filters against its own file list before merging, rather than reasoning about it. It changes `.github/workflows/**`, four Markdown docs, and `frontend/src/__tests__/deployWorkflow.test.ts`. That last path matches `^frontend/`, so `changes` reports `frontend=true backend=false`, and the run is `changes` → `deploy-frontend` → `verify`, with `deploy-backend` skipped.

   The deploy is a no-op in substance — a test file is not in the bundle, so the site is rebuilt and re-synced identically — but the *jobs run*, and that is the correct expectation to hold going in.

   An earlier draft of this item claimed the merge would deploy nothing, reasoning from "it is a workflow change" while parenthetically noting "(plus a test and docs)" without following through on what the filter does with a path under `frontend/`. That is the same failure mode as every other defect on this branch: a claim about the code that was never executed against the code. The filters are two `grep -E` invocations; running them takes ten seconds and settles it.
