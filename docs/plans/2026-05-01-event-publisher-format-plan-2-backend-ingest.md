...
- **Lambda DLQ / Errors alarm** — EventBridge silently drops the invocation after 2 retries. The CloudWatch log group is in place, but a `dead_letter_config` SQS queue or a `aws_cloudwatch_metric_alarm` on the `Errors` metric would proactively surface repeated failures.
