...
| 8 | #288 | Same 2026-10-01 date as #186, and #186's fix is one of its two halves. Web's behaviour is the correct one by its own recorded reasoning (`landingState.ts:103-107` rejected exactly iOS's rule). Also: ~~**no iOS UI test covers the off-season landing at all** — the fixture serves a single-year manifest~~ — *superseded 2026-08-31: `UITestFixture` now serves a three-season manifest and `YearNavigationUITests` covers both the pre-season and post-season landings.* |
